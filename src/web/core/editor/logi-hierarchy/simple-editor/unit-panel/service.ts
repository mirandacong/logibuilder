...
import {UnitPanelComponent, UNIT_PANEL_DATA} from './component'
...
export class UnitPanelService implements OnDestroy {
    public constructor(
    ...
    public openPanel(targetElement: HTMLElement, panelData: HoverInfo): void {
        this._targetElement = targetElement
        this._attachOverlay(panelData)
    }

    public hasAttached(element: HTMLElement): boolean {
        return this._targetElement === element && this._overlayAttached
    }
    ...
    private _componentRef?: ComponentRef<UnitPanelComponent>
    private _closingActionsSubs?: Subscription
    private _viewportSubscription = Subscription.EMPTY
    private _positionStrategy?: FlexibleConnectedPositionStrategy

    private _attachOverlay(data: HoverInfo): void {
        if (!this._overlayRef) {
        ...
        if (this._overlayRef && !this._overlayRef.hasAttached()) {
            const tokens = new WeakMap<InjectionToken<string>,
                HoverInfo>([[UNIT_PANEL_DATA, data]])
            const injector = new PortalInjector(this._injector, tokens)
            const portal
                = new ComponentPortal(UnitPanelComponent, undefined, injector)
