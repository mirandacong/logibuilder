...
function mergeChildren(children: readonly Node[]): readonly Node[] {
...
export function measure(
    ...
    rows: number,
    contentNodes: readonly Node[],
    fixedContent: readonly HTMLElement[],
    ellipsisStr: string,
    suffixStr = '',
): { readonly contentNodes: readonly Node[]; readonly text: string; readonly ellipsis: boolean } {
  ...
  // Get origin style
    ...
    const maxHeight = Math.round(lineHeight * (rows + 1) + pxToNumber(
        originStyle.paddingTop,
  ...
  // We should clone the childNode since they're controlled by React and we can't reuse it without warning
    const childNodes: ChildNode[] = Array.prototype.slice
        .apply(ellipsisContainer.childNodes[0].childNodes[0]
            .cloneNode(true).childNodes,
        )
        .filter(({nodeType}: ChildNode) => nodeType !== COMMENT_NODE)
    const fixedNodes: ChildNode[] = Array.prototype.slice.apply(
        ellipsisContainer.childNodes[0].childNodes[1]
            .cloneNode(true).childNodes,
    )
    ellipsisContainer.removeChild(container)
