...
import {ActionBuilder} from './add_label'

describe('test add label action', (): void => {
    ...
    it('add label in rows', (): void => {
        ...
        expect(newRow.labels).toEqual(['1', '2'])
        expect(newRow2.labels).toEqual(['2'])
    })
    it('add label in cols', (): void => {
        const action = new ActionBuilder()
        ...
        const newTable = service.book.sheets[0].tree[0]
        assertIsTable(newTable)
        expect(newTable.cols[0].labels).toEqual(['1', '2'])
    })
})

function mockService(): readonly [Readonly<EditorService>, Readonly<Table>] {
    const row = new RowBuilder().name('row').labels(['1']).build()
...
function buildCols(): readonly Readonly<Column>[] {
    return [new ColumnBuilder().name('col').labels(['1']).build()]
}
