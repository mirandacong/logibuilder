...
import {ActionBuilder} from './modify_label'

describe('test modify label action', (): void => {
    ...
    it('modify label in rows', (): void => {
        ...
        expect(newRow.labels).toEqual(['1', '4', '3'])
        expect(newRow2.labels).toEqual(['4'])
    })
    it('modify label in cols', (): void => {
        const action = new ActionBuilder()
        ...
        const newTable = service.book.sheets[0].tree[0]
        assertIsTable(newTable)
        expect(newTable.cols[0].labels).toEqual(['1', '3'])
    })
})
